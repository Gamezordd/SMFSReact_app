...
class AdminBasic extends React.Component<IProps, IState> {
  ...
  handleCategoryAdd = () => {
    ...
    if (this.state.new_points)
      this.setState({
        breakup: this.state.breakup.concat({
          category: this.state.new_cat,
          points: parseInt(this.state.new_points),
        }),
      });
  ...
  render() {
    const addCategory = (
      <div className="row">
        ...
        <input
          onChange={this.handleChange}
          value={this.state.new_points ? this.state.new_points : ""}
          placeholder="Points"
          style={{ margin: "10px" }}
          className="col-md-6 col-xs-10"
          id="new_points"
    ...
    const formFields = (
      <Form>
        ...
        <Form.Group>
          ...
          <Dropdown>
            <Dropdown.Toggle>{this.state.class}</Dropdown.Toggle>
            <Dropdown.Menu>
              <Dropdown.Item onSelect={this.handleDropdown}>
                Technical
              </Dropdown.Item>
              <Dropdown.Item onSelect={this.handleDropdown}>
                Non-Technical
        ...
        <Form.Group>
          ...
          <div>
            {this.state.breakup && this.state.breakup.length > 0 && (
              <Table size="sm" style={{ width: "50%" }}>
                ...
                <tbody>
                  {this.state.breakup?.map(
                    (ele: { category: string; points: number }) => {
                      return (
                        <tr>
                          <td> {ele.category} </td>
                          <td> {ele.points} </td>
          ...
          {this.state.newCat ? (
            <Button variant="secondary" onClick={this.handleCategoryAdd}>
              Add
            </Button>
          ) : (
            <Button
        ...
        <Form.Group style={{ display: "flex", flexDirection: "row" }}>
          <Button
            variant={this.state.submission_success ? "success" : "primary"}
            disabled={
              this.state.name === "" ||
          ...
          >
            {this.state.submission_success ? "Post Created" : "Submit"}
          </Button>
        </Form.Group>
      </Form>
    );
